**Öğrenci Adı Soyadı:** Duru Korkmaz  
**Staj Dönemi:** 01.07.2026-10.07.2026 
**3. ve 4. Hafta Çalışma Amacı:** Modern bir web projesinin teknik, içerik, SEO, sosyal medya ve sürüm kontrolü süreçlerini bütüncül olarak öğrenmek ve AIHEXA projesi üzerinde uygulamak.

## Kullanılacak Teknolojiler
* Git ve GitHub
* Spring Boot
* React
* WordPress ve SEO
* Sosyal Medya Yönetim Araçları

## Öğrenilecek Konular
1. Sürüm kontrolü ve GitHub disiplini
2. Backend, Frontend ve Veritabanı mimarisi
3. RESTful API tasarımı ve veri akışı
4. Dijital pazarlama, SEO ve sosyal medya entegrasyonu

## Teslim Edilecek Dosyalar
* README.md
* week-3-git-github.md
* week-3-spring-boot.md
* week-3-react.md
* week-3-wordpress.md
* week-3-social-media.md
* week-3-seo.md
* week-3-aihexa-module-analysis.md
* week-3-weekly-report.md
* week-4-project-plan.md
* week-4-api-analysis.md
* week-4-react-form-analysis.md
* week-4-wordpress-seo-analysis.md
* week-4-social-media-calendar.md
* week-4-final-report.md
